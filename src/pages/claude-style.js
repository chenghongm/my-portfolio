...
## Who is Chenghong
Chenghong is a full-stack developer with deep experience in Laravel/PHP backend systems and React frontend. Works across East and West Coast time zones. Currently focused on AI tooling and LLM engineering.

## Technical Stack
- Laravel / PHP: Primary stack. Production-grade APIs, queue workers, Eloquent ORM, MySQL.
- React: Frontend development, component architecture.
- MySQL: Schema design, charset/collation debugging, complex queries.
- Python: Data pipelines, LLM tooling, scripting.
- Node / JavaScript: Browser extensions, frontend logic.
- LLM / MLX: Local model fine-tuning with MLX framework on Apple Silicon. Llama 3 8B.
- Claude API / OpenAI API: Integration, classification pipelines, prompt engineering.

## Key Projects
1. Local LLM Training Pipeline — end-to-end pipeline: export conversation data from Claude, Gemini, GPT, Grok → score → filter → clean/desensitize → convert → fine-tune with MLX. Key architectural insight: scoring must happen BEFORE desensitization to avoid placeholder leakage into training data. Working adapter: local_claude_final2.

2. AI Chat Workbench Browser Extension — injects branch-marking UI into Claude, Gemini, ChatGPT, and Grok simultaneously. Platform-specific DOM adapters. Zero external dependencies.

3. AI-assistant QPR (Quarterly Progress Report) System Upgrade — Laravel queue architecture with timezone-aware scheduling. Debugged UTC date-shifting bugs (JavaScript ISO string interpretation), MySQL charset/collation conflicts, Eloquent belongsToMany relationship edge cases. Production deployed.

4. Multi-model Scoring Pipeline — two-stage classification: Claude API (Haiku) for topic judgment outperforms GPT; GPT for format summarization outperforms Claude. Each model used where it's stronger.

## Work Style
- Scope first: break tasks by complexity before writing code
- Iterate tight: file by file, line by line, fresh context when AI goes in circles
- Ship then refine: production first, polish second
- Uses AI tools extensively but with discipline — directs to specific files and line numbers

## Personality
Direct. Systems-oriented. Thinks in incentives and design decisions. Enjoys reverse-engineering observed behavior. Tests AI models comparatively. Calls out hedging and imprecision. Has a dry sense of humor. Not naive, but genuinely curious.

## How to respond
- Keep answers concise and terminal-appropriate (2-5 lines usually)
- If asked about something not in this context, say so directly — don't hallucinate
- You can be slightly witty but stay professional
- No markdown, no bullet points in responses — plain text only
- If asked "what can I ask", list a few example topics`;
  ...
  return (
    <div className={styles.container}>
      ...
      <nav className={styles.nav}>
        <a href="#" className={styles.navLogo}>Chenghong Meng</a>
        <ul className={styles.navLinks}>
          <li><a href="#about">About</a></li>
          <li><a href="#work">Projects</a></li>
          <li><a href="#process">Attitude</a></li>
      ...
      <section id="hero" className={styles.hero}>
        <div className={styles.winGhost}>
          <div className={styles.winGhostWindow} style={{ '--rot': '-3deg' }}>
          ...
          <div className={styles.winGhostWindow} style={{ '--rot': '2deg', marginLeft: '40px' }}>
            <div className={styles.winGhostTitle}>
              <div className={styles.winGhostDots}>
                <div className={`${styles.winGhostDot} ${styles.wgdR}`}></div>
                <div className={`${styles.winGhostDot} ${styles.wgdY}`}></div>
      ...
      <section id="about" className={styles.section} style={{ padding: 0 }}>
        <div className={styles.about}>
          <div className={`${styles.aboutLeft} ${styles.reveal}`}>
      ...
      <section id="work" className={styles.section}>
        ...
        {projects.map((proj) => (
          <div key={proj.id} className={`${styles.projectItem} ${styles.reveal}`}>
            <span className={styles.projNum}>{proj.id}</span>
            <div>
      ...
      <section id="process" className={styles.section}>
        ...
        <div className={styles.processGrid}>
          {workStyles.map((style) => (
            <div key={style.num} className={`${styles.processCard} ${styles.reveal}`}>
      ...
      <section id="contact" className={styles.section}>
        ...
        <div className={`${styles.contactGrid} ${styles.reveal}`}>
          <a href="mailto:mengchh01@gmail.com" className={styles.contactLink}>mengchh01@gmail.com</a>
          <a href="https://github.com/chenghongm" className={styles.contactLink} target="_blank">⌥ GitHub</a>
          <a href="https://www.linkedin.com/in/chenghong-m-6ab022103" className={styles.contactLink} target="_blank">→ LinkedIn</a>
        </div>
      </section>

      <footer className={styles.footer}>
        <span>Chenghong Meng — Full-Stack Developer - San Francisco, CA</span>
        <div className={styles.easterTrigger} onClick={() => setIsEasterEggOpen(true)}>
